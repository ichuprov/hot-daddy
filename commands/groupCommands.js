import { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, ChannelType, PermissionsBitField, MessageFlags } from 'discord.js';
import { nanoid, sendDMAndReply, checkAdminPermission, getSetting, setSetting, getGroups, getGroupById, getGroupsByCreatorId, insertGroup, updateGroupAnnouncementMessageId, updateGroupPrivateChannelAndStatus, getAcceptedUsers, insertAccepted, removeAccepted, insertApplicant, getApplicant, removeApplicant, getApplicantsForGroup, removeAllApplicantsForGroup, getInterests, upsertInterests } from '../db.js';
  ...
  return new ModalBuilder()
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('interests').setLabel('Enter your interests (comma separated)').setStyle(TextInputStyle.Paragraph).setRequired(true)
      )
    );
}
...
function buildApplyModal(groupName, groupId) {
  return new ModalBuilder()
    ...
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId('reason').setLabel('Why do you want to join this group?').setStyle(TextInputStyle.Paragraph).setRequired(true)
      )
    );
}

async function createPrivateGroupChannel(guild, group, acceptedIds, parentCategoryId) {
  const channelName = group.groupName.toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') + '-channel';
  const permissions = [
...
async function sendGroupAnnouncement(group, user, client, announceChannelId) {
  ...
  const applyButton = new ButtonBuilder().setCustomId(`apply_${group.id}`).setLabel('✨ Apply ✨').setStyle(ButtonStyle.Primary);
...
async function disableApplyButton(group, client, announceChannelId) {
  ...
  try {
    ...
    const disabledRow = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('apply_disabled').setLabel('✨ Group Full ✨').setStyle(ButtonStyle.Secondary).setDisabled(true)
    );
...
export async function handleCreateGroupSlash(interaction) {
  const isAllowed = getSetting('group_creation_allowed');
  if (isAllowed === 'false') {
    return interaction.reply({ content: '⛔ Group creation is currently disabled by an administrator.', flags: MessageFlags.Ephemeral });
  }
  await interaction.showModal(buildCreateGroupModal());
}

export async function handleListApplicationsSlash(interaction) {
  const creatorId = interaction.user.id;
  const creatorGroups = getGroupsByCreatorId(creatorId);
  if (creatorGroups.length === 0) {
    return interaction.reply({ content: '🧙 You haven\'t created any groups, so you have no applications to manage.', flags: MessageFlags.Ephemeral });
  }
    ...
    if (applicants.length > 0) {
      ...
      for (const applicant of applicants) {
        ...
        const applicationButtons = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`accept_${group.id}_${applicant.userId}`).setLabel('Accept').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`reject_${group.id}_${applicant.userId}`).setLabel('Reject').setStyle(ButtonStyle.Danger)
        );
        try {
          ...
          console.error(`Could not send DM to user ${creatorId}.`);
          await interaction.editReply({ content: '❌ I tried to send your applications to your DMs, but they are disabled.'});
          return;
        }
      }
    }
...
export async function handleListMyGroupsSlash(interaction) {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const creatorId = interaction.user.id;
  const myGroups = getGroupsByCreatorId(creatorId);
  if (myGroups.length === 0) {
    return interaction.followUp({ content: "🧙 You haven't created any groups yet. Use `/creategroup` to start one!", flags: MessageFlags.Ephemeral });
  }
...
export async function handleAllowGroupCreationSlash(interaction) {
...
export async function handleStopGroupCreationSlash(interaction) {
  if (!await checkAdminPermission(interaction)) return;
...
export async function handleRegisterInterestsSlash(interaction) {
  await interaction.showModal(buildRegisterInterestsModal());
}

export async function handleListGroupsSlash(interaction) {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const groups = getGroups();
  if (groups.length === 0) {
    await interaction.user.send('🧙 No groups found.');
    return interaction.followUp({ content: '🧙 Group list sent to your DMs (or no groups found).', flags: MessageFlags.Ephemeral });
  }
  let output = '🧙 **Group List:**\n\n';
  groups.forEach((g, index) => {
    output += `**Group ID:** ${g.id}\n**Name:** ${g.groupName}\n**Creator:** ${g.creatorName}\n**Member Count:** ${g.memberCount}\n**Status:** ${g.isFull ? 'Complete' : 'Pending'}\n`;
    if (groups.length > 1 && index !== groups.length - 1) output += '----------------------\n';
  });
  await sendDMAndReply(interaction, output, '🧙 Group list sent to your DMs!');
}

export async function handleForceGroupSlash(interaction, announceChannelId, groupCategoryId) {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  const groupId = interaction.options.getString('groupid');
  const group = getGroupById(groupId);
  if (!group) {
    return interaction.followUp({ content: '🧙 Group not found.', flags: MessageFlags.Ephemeral });
  }
  if (group.isFull) {
    return interaction.followUp({ content: '🧙 Group is already marked as complete.', flags: MessageFlags.Ephemeral });
  }
  try {
...
export async function handleShowApplicantsSlash(interaction) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const creatorId = interaction.user.id;
    const creatorGroups = getGroupsByCreatorId(creatorId);
    if (creatorGroups.length === 0) {
        return interaction.user.send('🧙 You haven\'t created any groups yet. Use `/creategroup` to get started!');
    }
    let foundApplicants = false;
    for (const group of creatorGroups) {
        const applicants = getApplicantsForGroup(group.id);
        if (applicants.length > 0) {
            ...
            for (const applicant of applicants) {
                ...
                await interaction.user.send({ content: `--- **Applicant for Group: ${group.groupName} (ID: ${group.id})** ---\n**Applicant:** ${applicant.userName} (<@${applicant.userId}>)\n**Reason:** ${applicant.reason}\n**Interests:** ${interests}`, components: [applicationButtons]
...
export async function handleCreateGroupModalSubmit(interaction, client, announceChannelId) {
  ...
  insertGroup({ id: groupId, creatorId: interaction.user.id, creatorName: interaction.user.username, groupName, description, memberCount, topics });
...
export async function handleRegisterInterestsModalSubmit(interaction) {
...
export async function handleApplyModalSubmit(interaction, client) {
  ...
  if (creatorUser) {
    ...
    const applicationButtons = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`accept_${group.id}_${interaction.user.id}`).setLabel('Accept').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`reject_${group.id}_${interaction.user.id}`).setLabel('Reject').setStyle(ButtonStyle.Danger)
    );
    creatorUser.send({ content: `🧙 **${interaction.user.username}** applied to your group **${group.groupName}**.\nReason: ${reason}\nInterests: ${interests}`, components: [applicationButtons] }).catch(() => { });
  }
  return interaction.reply({ content: 'Your application has been sent to the group creator.', flags: MessageFlags.Ephemeral });
}

export async function handleApplyButtonInteraction(interaction) {
...
export async function handleApplicationAction(interaction, client, announceChannelId, groupCategoryId) {
  ...
  if (group.isFull) {
      return interaction.update({ content: `${interaction.message.content}\n\n❌ **This group is already full.** You cannot accept more members.`, components: [] });
  }
  ...
  if (action === 'accept') {
    ...
    await interaction.update({ content: `${interaction.message.content}\n\n✅ You accepted **${applicant.username}**.`, components: [] });
    ...
    await interaction.update({ content: `${interaction.message.content}\n\n❌ You rejected **${applicant.username}**'s application.`, components: [] });
  }
}
...
export function getSlashCommands() {
  return [
    ...
    new SlashCommandBuilder().setName('showapplicants').setDescription('Shows pending applications for your groups (Admin only).'),
    new SlashCommandBuilder().setName('allowgroupcreation').setDescription('Allows users to create new groups (Admin only)').setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator),
    new SlashCommandBuilder().setName('stopgroupcreation').setDescription('Stops users from creating new groups (Admin only)').setDefaultMemberPermissions(PermissionsBitField.Flags.Administrator),
  ];
}